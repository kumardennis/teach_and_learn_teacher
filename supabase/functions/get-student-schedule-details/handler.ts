...
export const handler = async (req: Request) => {
	...
	try {
		...
		const lessonQueryString =
			`Lessons(${studentQueryString}, id, isInWaitingList, isAccepted, isChild)`;

		const { data, error } = await supabase.from('LessonSchedules').select(
			`*, Subcategories(id, name, description, picture, categoryId) ,${lessonScheduleTimeQueryString}, ${lessonQueryString}, ${addressQueryString}`,
