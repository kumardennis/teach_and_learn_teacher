...
import 'package:teach_and_learn_teacher/feature_ViewSchedules/getxControllers/previous_schedules_controller.dart';
...
class PreviousSchedulesBrief extends HookWidget {
  final UserController userController = Get.put(UserController());

  final PreviousSchedulesController previousSchedulesController =
      Get.put(PreviousSchedulesController());

  ValueNotifier<List<ScheduleReponseModel>> schedules = useState([]);

  PreviousSchedulesBrief({Key? key}) : super(key: key);
    ...
    Future<void> getSchedules(teacherId, accessToken) async {
      try {
        final results = await Supabase.instance.client.functions.invoke(
            ...
            body: {"teacherId": teacherId, "isInPast": true});
        ...
        if (results.data['isRequestSuccessful']) {
          ...
          previousSchedulesController.loadPreviousSchedules(schedulesList);
        } else {
                      ...
                      child: ListView(
                        ...
                        children: schedules.value
                            ...
                            .map((schedule) => GestureDetector(
                                  onTap: () {
                                    Get.toNamed('/previous-schedules',
                                        arguments: {
